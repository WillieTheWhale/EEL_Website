...
const CircuitGridMatrix = {
    ...
    // Animation timing
    packetSpeed: 0.2, // Progress per second (reduced 50% from 0.4)
    lastUpdateTime: 0,
...
function createEngineeringStructures() {
    ...
    // Only the Quadcopter Drone is created (all other shapes removed)
    const quadcopter = createQuadcopter();
    quadcopter.position.set(0, 20, -55); // Start at first waypoint position
    quadcopter.userData.originalPosition = quadcopter.position.clone();
    scene.add(quadcopter);
    structures.push(quadcopter);

    // Figure-8 infinity loop waypoints covering entire page, avoiding center
    // Path is a continuous loop that returns to starting point smoothly
    // All Y values shifted down by 35 to keep drone on screen

    // Figure-8 path avoiding center (radius ~40) and covering page area
    const flightPath = [
        { x: 0, y: 20, z: -55 },       // Start: Top center crossing
        { x: 40, y: 25, z: -52 },      // Transition to right
        { x: 70, y: 40, z: -50 },      // Top-right corner approach
        { x: 85, y: 20, z: -45 },      // Far right edge
        { x: 75, y: 0, z: -48 },       // Descending right side
        { x: 40, y: 15, z: -52 },      // Safe corridor above center
        { x: -40, y: 15, z: -52 },     // Cross to left side above center
        { x: -75, y: 0, z: -48 },      // Descending left side
        { x: -85, y: -40, z: -45 },    // Far bottom-left edge
        { x: -70, y: -55, z: -50 },    // Bottom-left corner approach
        { x: -40, y: -45, z: -65 },    // Safe corridor below center (deeper)
        { x: 0, y: -40, z: -70 },      // Bottom center crossing (deepest point)
        { x: 40, y: -45, z: -65 },     // Right side below center
        { x: 70, y: -55, z: -50 },     // Bottom-right corner
        { x: 85, y: -40, z: -48 },     // Far right edge (lower)
        { x: 70, y: -15, z: -52 },     // Ascending right side
        { x: 40, y: 10, z: -54 }       // Return to top (approaches start)
    ];

    flightPath.forEach(wp => {
        quadcopter.userData.waypoints.push(new THREE.Vector3(wp.x, wp.y, wp.z));
    });
    // Modulo in animation loop handles smooth wrap from last waypoint back to first
            ...
            case 'circuit_trace':
                if (structure.userData.pulses) {
                    structure.userData.pulses.forEach((pulse, idx) => {
                        pulse.userData.progress += deltaTime * 0.15;
